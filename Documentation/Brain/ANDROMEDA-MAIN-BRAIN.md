...
The active engineering focus is [[Gateway Plan|Gateway: Networking + MCP + LLM Proxy]]. The first implemented Hummingbird surface is [[Autocache Gateway|Autocache Swift]]: Anthropic prompt-cache injection with ROI headers. Memory architecture is recorded but paused while the runtime spine expands beyond Autocache.
...
Autocache Anthropic proxy is bootstrapped. Next prove the typed request path with mocks before broadening providers:

`request → encrypted append → privacy filter → policy → capability route → mock stream → sanitized trace`

Then expand beyond `/v1/messages` into OpenAI-compatible routing and MCP.
