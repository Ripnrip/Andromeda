...
> Andromeda Gateway is a local-first, Swift-native networking and policy plane for model APIs, MCP, privacy, evidence capture, and observability. The Autocache Anthropic surface is now implemented in Swift (`AndromedaAutoCache` + Hummingbird); the broader OpenAI/MCP/evidence plane remains directional.
...
1. **Foundation:** Swift package, module boundaries, typed IDs/errors, Argument Parser, Hummingbird, NIO, clocks, mocks, and structured logging. *(partial: package + CLI + Autocache live)*
2. **Autocache Anthropic surface:** heuristic token analysis, cache_control injection, ROI headers, `/v1/messages` proxy. *(implemented — see [[Autocache Gateway]])*
3. **Gateway spine:** compiled topology, broad compatibility DTOs, provider registry, capability router, SSE, cancellation, and typed failure mapping.
4. **Evidence and privacy:** encrypted archive, sanitizer pipeline, trace reconstruction, structural diffs, imports, exports, and Phoenix OTLP.
5. **MCP:** client/server transports, permission engine, approval records, containment, and trace propagation.
6. **Optimization extensions:** prefix-drift detection, reversible compression/retrieval; Autocache already covers Anthropic cache stabilization and honest savings headers.
