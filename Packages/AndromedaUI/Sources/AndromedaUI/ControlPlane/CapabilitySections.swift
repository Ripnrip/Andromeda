...
public struct ModelsSection: View {
    ...
    private var models: some View {
        VStack(alignment: .leading, spacing: 3) {
            HStack(spacing: 10) {
                ...
                Text("44 models").font(AndromedaFont.mono(12)).foregroundStyle(Color.andromedaMuted)
            ...
            ForEach(filtered) { m in
                HStack(spacing: 12) {
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("andromeda gateway").font(AndromedaFont.ui(11)).foregroundStyle(Color.andromedaMuted)
                    }
                    Spacer()
                    DotStatus(text: m.tier, color: m.tierColor)
                    pill("curl", filled: false)
    ...
    private func pill(_ t: String, filled: Bool) -> some View {
        Text(t).font(filled ? AndromedaFont.ui(11, .medium) : AndromedaFont.mono(11))
            .foregroundStyle(filled ? .white : Color.andromedaMuted)
            .padding(.horizontal, 13).padding(.vertical, 5)
            .background(Capsule().fill(filled ? Color(red: 0.42, green: 0.36, blue: 0.91) : .clear)
        ...
        VStack(alignment: .leading, spacing: 3) {
            ForEach(ControlPlaneData.speed) { s in
                HStack(spacing: 14) {
                    ...
                    Spacer()
                    Text(s.latency).font(AndromedaFont.mono(13)).foregroundStyle(Color.andromedaLive).frame(width: 80, alignment: .trailing)
                    Text(s.tps).font(AndromedaFont.mono(13)).foregroundStyle(Color.andromedaLive).frame(width: 70, alignment: .trailing)
                }
    ...
    private var health: some View {
        let cards: [(String, String, Color, String)] = [
            ("uptime · 24h", "99.8%", .andromedaLive, "1 breaker trip, auto-recovered"),
            ("p50 latency", "1.2s", .andromedaTeal, "across warm routes"),
            ("fallbacks fired", "12", .andromedaAmber, "provider → provider, 24h"),
            ("tokens · today", "4.1M", .andromedaInk, "62% reasoning · 31% fast"),
            ("cache hit", "38%", .andromedaTeal, "autocache, prompt-prefix"),
            ("spend · today", "$18.40", .andromedaInk, "under $40 daily cap"),
        ]
