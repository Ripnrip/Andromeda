...
/// Stable fixture for naming each image in the outcome matrix.
///
/// Kept outside the `@MainActor` suite so `@Test(arguments:)` can read it
/// synchronously — MainActor-isolated statics become async under Swift Testing.
struct HUDComponentOutcomeFixture: Sendable {
    let name: String
    let outcome: HUDOutcome
}

/// Enumerates every visual branch of `HUDOutcomeView`, including all four success rows.
enum HUDComponentOutcomeFixtures {
    static let all: [HUDComponentOutcomeFixture] = [
...
/// impostors. Tip CI commits with `[record-snapshots]` so macos-15 + Xcode 16.4 write baselines.
...
struct HUDComponentSnapshotTests {
    ...
    @Test("HUDOutcomeView all states", arguments: HUDComponentOutcomeFixtures.all, [("light", true), ("dark", false)])
    func outcomeSnapshots(fixture: HUDComponentOutcomeFixture, appearance: (String, Bool)) {
